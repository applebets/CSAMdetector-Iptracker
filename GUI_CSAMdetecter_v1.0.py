...
# Injected functions from torrent_scraper_site.py
from torrent_scraper_site import get_detail_links, parse_detail_page

# === Global Tracker Object ===
tracker = None
current_torrent_name = ""
    ...
    try:
        ...
        for p in peers.values():
            output_box.insert("", "end", values=(
                f"{p['ip']}:{p['port']}", p['hostname'], p['asn'],
                p['client'], f"{p['progress']}%", p['country'], "YES" if p['is_vpn'] else "NO"
            ))
        if auto_save.get():
            tracker.save_log()
    except Exception as e:
...
def toggle_remove_torrent():
    print("🧹 Torrent removal on exit:" if remove_on_exit.get() else "🚫 Torrent will be kept.")
...
tk.Button(control_frame, text="🔄 Refresh", command=refresh_peers, width=10).grid(row=0, column=1, padx=5)

auto_save = tk.BooleanVar()
remove_on_exit = tk.BooleanVar()

tk.Checkbutton(control_frame, text="💾 Save log after each refresh", variable=auto_save).grid(row=0, column=2, padx=5)
tk.Checkbutton(control_frame, text="🧹 Remove torrent on exit", variable=remove_on_exit, command=toggle_remove_torrent).grid(row=0, column=3, padx=5)

# === Table Output ===
columns = ("IP:Port", "Hostname", "ASN Info", "Client", "Progress", "Country", "VPN")
output_box = ttk.Treeview(root, columns=columns, show="headings", height=15)
for col in columns:
    output_box.heading(col, text=col)
    output_box.column(col, width=120)
output_box.pack(pady=10, fill="x")
